...
const parserRules: ParserRule[] = [
  ...
  // Bold text rule
  {
    regex: /\*\*(.*?)\*\*/g,
    generator: (match) => (
      <DiscordBold key={`bold-${match.index}`}>
        {match[1]}
      </DiscordBold>
    ),
  },
  ...
  // Italic text rule
  {
    regex: /\*(.*?)\*|_(.*?)_/g,
    generator: (match) => (
      <DiscordItalic key={`italic-${match.index}`}>
        {match[1] || match[2]}
      </DiscordItalic>
    ),
  },
  ...
  // Underlined text rule
  {
    regex: /__(.*?)__/g,
    generator: (match) => (
      <DiscordUnderlined key={`underline-${match.index}`}>
        {match[1]}
      </DiscordUnderlined>
    ),
  },
  
  // Strikethrough text rule
  {
    regex: /~~(.*?)~~/g,
    generator: (match) => (
      <span key={`strike-${match.index}`} style={{ textDecoration: 'line-through' }}>
        {match[1]}
      </span>
    ),
  },
  ...
  // Inline code rule
  {
    regex: /`([^`]+)`/g,
    generator: (match) => (
      <DiscordCode key={`code-${match.index}`}>
        {match[1]}
      </DiscordCode>
    ),
  },
  ...
  // Quote rule
  {
    regex: /^> (.+)$/gm,
    generator: (match) => (
      <DiscordQuote key={`quote-${match.index}`}>
        {match[1]}
      </DiscordQuote>
    ),
  },
